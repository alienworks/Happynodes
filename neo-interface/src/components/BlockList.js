...
class BlockList extends Component {
    render() {
        ...
        if (blocklist.pending) {
        ...
        } else if (blocklist.fulfilled) {
            console.log(blocklist.value);
            let data = blocklist.value.data.result;
            console.log(data)
            ...
            return (
                    ...
                    <div className="jumbotron nodes">
                        ...
                        {data.map((item, i) =>
                        <tr key={i}>
                        <td><a href={"../block/"+item.index}>{item.index}</a></td>
                        <td>{item.size}</td>
                        <td>{item.time}</td>
                        <td>{item.txcount}</td>
                        </tr>
...
export default connect((props) => ({
    blocklist: {
        url: config.api_url.replace('/redis', '').concat(`/blocks/30/1`),
