...
class Addressnfo extends Component {
    render() {
        ...
        if (addressinfo.pending) {
            ...
            console.log("addressinfo", addressinfo.value.data.result[0])

            const data = addressinfo.value.data.result[0];
...
export default connect((props) => ({
    addressinfo: { url: config.api_url.replace('/redis', '').concat(`/block/${props.block_id}`) },
}))(AddressInfo)