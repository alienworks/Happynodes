...
class AssetList extends Component {
    render() {
        ...
        if (assetlist.pending) {
        ...
        } else if (assetlist.fulfilled) {
            console.log(assetlist.value);
            let data = assetlist.value.data.result;
            console.log(data)
            ...
            return (
                    ...
                    <div className="jumbotron nodes">
                        ...
                        {data.map((item, i) =>
                        <tr key={i}>
                        <td><a href={"../asset/"+item.id}>{item.id}</a></td>
                        <td>{item.name.length > 1 ? item.name[1].name : item.name[0].name}</td>
                        <td>{item.amount}</td>
...
export default connect((props) => ({
    assetlist: {
        url: config.api_url.replace('/redis', '').concat(`/allassets`),
