...
router.get('/nodes', cache('1 minute'), function (req, res, next) {
	...
	pool.connect()
		.then(client => {
			...
			coalesce(
				all_time_stab.all_times_stability,
				0
			) as all_times_stability
		from
			address addr
		...
		left join (
		SELECT 
		all_time_stab_cal.address_id, 
		all_time_stab_cal.total_pings_online,  
		all_time_stab_cal.total_pings, 
		all_time_stab_cal.total_pings_online*1.0/all_time_stab_cal.total_pings AS all_times_stability
		FROM
		(SELECT
		  oh.address_id,
		  SUM(CASE
			WHEN oh.online = TRUE THEN 1
			ELSE 0
		  END) AS total_pings_online,
			count(oh.online) AS total_pings
		FROM  online_history oh
		GROUP BY oh.address_id) all_time_stab_cal
		)	all_time_stab on
		all_time_stab.address_id = addr.id
		left join (
				select
					address_id,
					p2p_tcp_status
